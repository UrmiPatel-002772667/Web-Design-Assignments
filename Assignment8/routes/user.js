...
userRouter.post("/uploadImage/:email", async (req, res, next) => {
  try {
      const userEmail = req.params.email; // Get email from route parameter

      // Single file upload with a field name 'image'
      upload.single('image')(req, res, async (err) => { 
          ...
          res.status(200).json({ message: 'Image uploaded', filePath: updatedUser.imagePath });
      });
  } catch (error) {
